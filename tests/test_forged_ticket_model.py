...
class TestRoundTrip:
    ...
    # NOTE: SAPNode.sso2_check_result and its round-trip tests
    # used to live here.  The field was populated only by the
    # standalone /api/node/<sid>/sso2_profile_check endpoint,
    # which has been removed (the implicit check inside
    # extract_and_forge_ticket attaches its result to the FORGE
    # result dict instead, never to the node).  The orphaned
    # field + tests are gone as of the same commit.
