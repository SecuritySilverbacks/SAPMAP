            ...
            if not any(f.name.startswith("CVE-2022-22536")
                ...
                node.findings.append(Finding(
                    ...
                    description=(
                        ...
                        f"endpoint.  Three explanations from most likely "
                        f"to least:\n"
                        f"  (1) wdisp/additional_conn_close=1 is set "
                        f"(SAP Note 3138881 workaround — being deprecated "
                        f"per Note 3200257);\n"
                        f"  (2) an emergency security hotfix has been "
                        f"backported into this binary without bumping "
                        f"the PL number visible in `disp+work -V` / "
                        f"`sapwebdisp -V` / SAPMC banner — verify with "
                        f"the kernel hash against SAP's "
                        f"PatchLevelCheckBC tool;\n"
                        f"  (3) the WD's URL filter denied the probe's "
                        f"outer path (try a different outer_path that "
                        f"actually forwards to the backend).\n"
                        f"Either way the patch number is below SAP's "
                        f"declared fix line and the workaround is being "
                        f"deprecated — patching to the boundary in "
                        f"3123396 is the only durable fix."
                    ),
                    remediation=(
