...
def _derive_landscape_recommendations(state: SAPMAPState) -> list:
    ...
    # 3aa. CVE-2026-44756 — OVERPASS EPP kernel memory corruption (Note 3747649)
    epp_rce = sorted(
        n.sid for n in nodes
        if getattr(n, "cve_2026_44756_rce_confirmed", False))
    epp_dos = sorted(
        n.sid for n in nodes
        if getattr(n, "cve_2026_44756_dos_confirmed", False)
           and n.sid not in epp_rce)
    epp_vuln = sorted(
        n.sid for n in nodes
        if getattr(n, "cve_2026_44756_vulnerable", False)
           and n.sid not in epp_rce
           and n.sid not in epp_dos)
    if epp_rce or epp_dos or epp_vuln:
        scope_parts = []
        if epp_rce:
            scope_parts.append("DIAG RCE CONFIRMED: " + ", ".join(epp_rce))
        if epp_dos:
            scope_parts.append("HTTP DoS CONFIRMED: " + ", ".join(epp_dos))
        if epp_vuln:
            scope_parts.append("kernel PL below fix (not yet exploited): "
                                + ", ".join(epp_vuln))
        body = (
            "SAP Security Note 3747649 (CVE-2026-44756, CVSS 10.0 "
            "HotNews) — Three memory-safety bugs in the SAP kernel "
            "Extended Passport parser (`eppDeserialize`), reachable "
            "pre-auth on HTTP, RFC, and DIAG.  Bug C is a type-4 "
            "var-part item transcoded UTF-8→UTF-16 into a 0x430-byte "
            "stack buffer with no length cap — writes past the saved "
            "return address at rbp+8.  Impact per channel:\n"
            "  • HTTP  — Unauth ICM-worker DoS.  RIP-hijack primitive "
            "exists but ICM re-execs on crash so ASLR re-randomises "
            "on every attempt.\n"
            "  • RFC   — DoS class only (passport capped at 512B by "
            "kernel).\n"
            "  • DIAG  — Unauth arbitrary-command RCE.  Work processes "
            "fork from the dispatcher without execve, so text/libc "
            "base is stable for the instance's lifetime.  Given the "
            "three per-instance recon values (dw, libc, session-rdx), "
            "a two-stage COP chain calls `system(<cmd>)` in the "
            "target work process."
        )
        if epp_rce:
            body += (f"\n\nDIAG arbitrary-command RCE delivery signature "
                      f"confirmed on: {', '.join(epp_rce)} — the target's "
                      f"work process took the hijack and executed the "
                      f"attacker-supplied command through the two-stage "
                      f"COP chain.  Verify the on-target artefact "
                      f"(default: `/tmp/sapmap_cve_44756_proof.txt`).")
        if epp_dos:
            body += (f"\n\nHTTP DoS confirmed on: {', '.join(epp_dos)} — "
                      f"the ICM worker crashed on our bug-C payload; SAP "
                      f"auto-restarted the worker within seconds.  RCE "
                      f"remains available via the DIAG channel once "
                      f"per-instance bases have been captured.")
        body += (
            "\n\nFixed kernel patch levels: 7.93 PL412, 9.16 PL100, "
            "9.18 PL032, 9.19 PL017, 9.20 PL007.  Kernel binary patch "
            "— no functional workaround for the DIAG / RFC channels.  "
            "The Web Dispatcher workaround (SAP Note 3756304) covers "
            "the HTTP path only.  Network detection: the SAPMAP "
            "repository ships Suricata / Snort / YARA / Sigma rules "
            "under `tools/detection/cve-2026-44756/` alongside a "
            "12-pcap validation corpus (all rules verified against "
            "ground truth)."
        )
        items.append({
            "category": "Kernel patching",
            "scope": " | ".join(scope_parts),
            "title": "Patch CVE-2026-44756 (OVERPASS — EPP parser memory corruption)",
            "body": body,
            "refs": "SAP Security Note 3747649 (WD workaround: 3756304)",
        })
