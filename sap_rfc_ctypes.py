...
class RFC_PARAMETER_DESC(Structure):
    ...
    _fields_ = [
        ...
        ('type', c_uint),              # RFCTYPE  (comes first in sapnwrfc.h)
...
class RFCConnection:
    ...
    def _get_value(self, container, name_uc, abap_type, type_desc, uc_length, decimals):
        ...
        # -- CHAR and NUM: use RfcGetChars (RfcGetString only works for STRING/XSTRING) --
        if abap_type in (RFCTYPE_CHAR, RFCTYPE_NUM):
            char_count = uc_length // 2  # ucLength is in bytes, SAP_UC is 2 bytes
            if char_count == 0:
                char_count = 1
            buf = _create_uc_buffer(char_count + 1)
            error_info = RFC_ERROR_INFO()
            rc = self._sdk.RfcGetChars(
                container, name_uc, buf, char_count, byref(error_info)
            )
            _raise_on_error(error_info, 'RfcGetChars')
            return _uc_to_str(buf, char_count).rstrip()

        # -- STRING, UTCLONG, and all other types → read as string --
