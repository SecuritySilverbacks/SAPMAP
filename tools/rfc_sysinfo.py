#!/usr/bin/env python3
"""rfc_sysinfo.py — anonymous RFC_SYSTEM_INFO probe (pure stdlib).

Opens a CPIC/APPC conversation on the SAP gateway, sends the
anonymous connect naming RFC_SYSTEM_INFO, parses the 245-byte
RFCSI_EXPORT record from the reply.

    python3 rfc_sysinfo.py <host> [port] [--timeout 5] [--json]

No external deps — no pysap, no scapy, no NW RFC SDK, no
saprfclib.  Speaks SAP NI framing (4-byte big-endian length
prefix) directly over TCP.
...
Julian reverse-engineered the 80-byte APPC v6 header layout
against kernel 7.93 and demonstrated that the anonymous connect
path can dispatch RFC_SYSTEM_INFO, RFC_PING, and
SYSTEM_INVISIBLE_GUI pre-logon — returning real replies, not just
error banners.

This copy keeps only the RFC_SYSTEM_INFO path, drops the multi-FM
...
7.93.  Other kernels may need tweaks — the tool reports which
stage failed so you can tell whether it's a handshake problem or
a reply-parse problem.

SAProuter tunneling is not implemented in this build.  If you
need one, extend the NI transport to speak NI_ROUTE first.
"""
