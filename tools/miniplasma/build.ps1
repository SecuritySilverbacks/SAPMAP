...
.DESCRIPTION
    ...
    The resulting blob ships with SAPMAP — operators don't need to
...
if (-not $Nuget) {
    Write-Host "[*] nuget.exe    : not on PATH — downloading to $Here\nuget.exe"
    Invoke-WebRequest -Uri "https://dist.nuget.org/win-x86-commandline/latest/nuget.exe" -OutFile (Join-Path $Here "nuget.exe")
    $NugetPath = Join-Path $Here "nuget.exe"
} else {
...
for ($i = 0; $i -lt $bytes.Length; $i += 40) {
    ...
    for ($j = 0; $j -lt $count; $j++) {
        $sb.Append("{0:x2}" -f $bytes[$i + $j]) | Out-Null
    }
    $sb.Append('"') | Out-Null
...
$header = @"
"""Auto-generated by tools/miniplasma/build.ps1 — do not hand-edit.

Holds the SAPMAP-vendored MiniPlasma binary (.NET 4.7.2, Costura-merged,
ConfuserEx-obfuscated) as a hex blob.  The blob is delivered to the
target via SAPXPG cmd.exe + certutil -decode, written to
%TEMP%\mp_bin.exe, then run.  See modules/exploitation/sapmap_miniplasma.py.
"""

MINIPLASMA_SHA256 = "$sha"
MINIPLASMA_SIZE   = $size
...
[System.IO.File]::WriteAllText($BlobPy, $header, [System.Text.UTF8Encoding]::new($false))
